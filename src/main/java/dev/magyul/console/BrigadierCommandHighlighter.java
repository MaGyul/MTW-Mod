...
public class BrigadierCommandHighlighter implements Highlighter {
    ...
    public BrigadierCommandHighlighter(final @NotNull MinecraftDedicatedServer server) {
        this.server = server;
    ...
    public AttributedString highlight(LineReader reader, String buffer) {
        ...
        final var results = this.server.getCommandManager().getDispatcher().parse(BrigadierCommandCompleter.prepareStringReader(buffer), this.serverCommandSource.get());
        int pos = 0;
        ...
        int component = -1;
        for (final var node : results.getContext().getLastChild().getNodes()) {
            ...
            if (node.getNode() instanceof LiteralCommandNode<ServerCommandSource>) {
